...
void quickhull(ExecutionPolicy&& exec, const pointVec_t &points, pointVec_t &hull) {
    if (points.size() < 2) {
        hull.insert(hull.end(), points.cbegin(), points.cend());
        return;
    }
    //Find left and right most points, they will be in the convex hull
    auto minmaxx = std::minmax_element(exec, points.cbegin(), points.cend());
    ...
    divide_and_conquer(exec, points.cbegin(), points.cend(), hull, p1, p2, p1);
}
