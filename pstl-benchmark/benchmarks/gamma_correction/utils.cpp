...
bool image::compare(const image& other) const {
  if (my_data.size() != other.my_data.size())
    return false;
  for (size_t i = 0; i < my_data.size(); ++i) {
    if (my_data[i].value != other.my_data[i].value) {
      std::cerr << "ERROR at index " << i << ": "
		<< std::hex << my_data[i].value << " != "
		<< other.my_data[i].value << std::endl;
      return false;
    }
  }
  return true;
...
void image::read(const char* fname) {
  ...
  if (!stream.is_open())
    ABORT_WITH_ERROR(std::string("Could not open ") + fname);

  stream.read((char*)&file.type, file.sizeRest);
  stream.read((char*)&info, info.size);
  assert(info.sizeImage == my_data.size() * sizeof(my_data[0]));
  stream.read((char*)my_data[0].bgra, my_data.size()*sizeof(my_data[0]));
  stream.close();
}
